...
\paragraph{Infer normal from depth.} 
\label{chap:d2n}
...
&N(x_i) = \ve{n} / \|\ve{n}\|_2
\end{align}
We show the process of calculating normal direction in Figure \ref{fig:d2n}. 
...
Additionally, with normal representation, we additionally require smoothness over neighboring normal values, which provides high order interactive between pixels. Formally, the smoothness for normal haves the same form as $\scr{E}_{s}$ in \equref{eqn:regular}, while we apply the first order gradient, \ie $\scr{E}_{s}(N, 1)$. 

Last but not the least, matching corresponding pixels between frames is another central factor to find correct geometry. Additional to photometric error from matching pixel colors, matching image gradient is more robust to lighting variations, which was frequently applied in computing optical flow~\cite{DBLP:journals/corr/Li17n}. 
