...
def load_gt_disp_kitti(path):
    gt_disparities = []
    for i in range(200):
        disp = sm.imread(path + "/training/disp_noc_0/" + str(i).zfill(6) + "_10.png", -1)
...
def load_gt_disp_make3d(path, test_fn):
    gt_disparities = []
    with open(test_fn) as f:
        for line in f:
            disparity = sm.imread(path+"/depth_gt/depth_sph_corr"+line.strip().split("img")[1])
            disparity = disparity.astype(np.float32) / 255.0
            gt_disparities.append(disparity)
    return gt_disparities

def load_gt_disp_nyuv2(path, test_fn):
    gt_depths = []
    with open(test_fn) as f:
        for line in f:
            depth = sm.imread(path + "raw_depth_gt/" + line.rstrip().split("/")[-1])
...
def convert_disps_to_depths_kitti(gt_disparities, pred_disparities):
    ...
    for i in range(len(gt_disparities)):
        ...
        pred_disp = sm.imresize(pred_disp, (height, width), mode='F')
        ...
        gt_depth = width_to_focal[width] * 0.54 / (gt_disp + 1e-4)
        pred_depth = width_to_focal[width] * 0.54 / (pred_disp + 1e-4)
        pred_depth = pred_disp
        # gt_depth = 1.0 / gt_disp
...
def normalize_median(pred, gt, mask):
    ratio = np.median(gt[mask]) / np.median(pred[mask])
    return pred*ratio, gt
###############################################################################
#######################  EIGEN
...
def resize2same(pred_maps, gt_maps):
    ## resize pred_map to gt_map size
    ## input should be numpy array with batch 
    pred_maps_resize = []  
    for i in range(pred_maps.shape[0]):
        pred_maps_resize.append(sm.imresize(pred_maps[i,:,:,:], gt_maps.shape[1:])/255.0)

    return np.array(pred_maps_resize), gt_maps
