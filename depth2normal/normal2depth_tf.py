...
def normal2depth_layer_batch(depth_map, normal_map, intrinsics, tgt_image, nei=1):
    ...
    # print x_ctr_tile.get_shape().as_list()
    # print cx_tile.get_shape().as_list()
    ## d_1 = d_0 * ((x_ctr-cx)/fx*normal_x + (y_ctr-cy)/fy*normal_y + normal_z) / ((x_0-cx)/fx*normal_x + (y_0-cy)/fy*normal_y + normal_z)
    ...
    # depth_map_x0 = numerator / denominator_x0 * depth_map
    # depth_map_x0 = (tf.clip_by_value(numerator / denominator_x0, 0.5, 2.0)+4.0) * depth_map
    ...
    # depth_map_x0y0 = (tf.sigmoid(numerator / denominator_x0y0 - 1.0) * 2.0 + 4.0) * depth_map
    # depth_map_x0y1 = (tf.sigmoid(numerator / denominator_x0y1 - 1.0) * 2.0 + 4.0) * depth_map
    # depth_map_x1y0 = (tf.sigmoid(numerator / denominator_x1y0 - 1.0) * 2.0 + 4.0) * depth_map
    # depth_map_x1y1 = (tf.sigmoid(numerator / denominator_x1y1 - 1.0) * 2.0 + 4.0) * depth_map
    ...
    # depth_map_x0y0 = tf.pad(depth_map_x0y0-1e3, padding_x0y0)+1e3
    # depth_map_x0y1 = tf.pad(depth_map_x0y1-1e3, padding_x0y1)+1e3
    # depth_map_x1y0 = tf.pad(depth_map_x1y0-1e3, padding_x1y0)+1e3
    # depth_map_x1y1 = tf.pad(depth_map_x1y1-1e3, padding_x1y1)+1e3
    ...
    # depth_map_avg = tf.pad(depth_map_avg[:,2*nei:-2*nei, 2*nei:-2*nei] - 1e3, [[0,0],[2*nei, 2*nei],[2*nei, 2*nei]])+1e3

    # depth_map_avg = tf.pad(tf.reduce_mean(tf.stack([depth_map_x0[:, 2*nei:-2*nei, 2*nei:-2*nei], depth_map_x1[:, 2*nei:-2*nei, 2*nei:-2*nei], 
    #                         depth_map_y0[:, 2*nei:-2*nei, 2*nei:-2*nei], depth_map_y1[:, 2*nei:-2*nei, 2*nei:-2*nei],
    #                         depth_map_x0y0[:, 2*nei:-2*nei, 2*nei:-2*nei], depth_map_x0y1[:, 2*nei:-2*nei, 2*nei:-2*nei],
    #                         depth_map_x1y0[:, 2*nei:-2*nei, 2*nei:-2*nei], depth_map_x1y1[:, 2*nei:-2*nei, 2*nei:-2*nei]]),0), 
    #                         [[0,0],[2*nei, 2*nei],[2*nei, 2*nei]])

    # return tf.reduce_mean([depth_map_x0, depth_map_y0, depth_map_x1, depth_map_y1], axis=0)
    # return [depth_map_x0, depth_map_y0, depth_map_x1, depth_map_y1]#, depth_map_x0y0, depth_map_x0y1, depth_map_x1y0, depth_map_x1y1
    # return depth_map_y0#tf.reduce_mean(weights,0)] 
    return depth_map_avg
    # return [depth_map_avg, img_grad_x0, tf.stack([weights_x0, weights_y0, weights_x1, weights_y1])]#, numerator, denominator_y0
