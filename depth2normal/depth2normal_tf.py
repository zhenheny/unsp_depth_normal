...
def depth2normal_layer(depth_map, intrinsics, inverse):
    ...
    # normals0 = tf.cross(diff_x1, diff_y1)
    ...
    # return normal_map, pts_3d_map_x0, pts_3d_map_x1, pts_3d_map_y0, pts_3d_map_y1, pts_3d_map_x0y0, pts_3d_map_x0y1, diff_x0, diff_y0, diff_x1, diff_y1, normals0, normals1, normals2, normals3, normal_vector
    return normal_map
...
def depth2normal_layer_batch(depth_map, intrinsics, inverse, nei=3):
    ...
    # ## generate masks to filter out too large depth difference
    # diff_depth_avg = tf.reduce_mean(tf.abs([diff_x0[:,-1], diff_x1[:,-1], diff_y0[:,-1], diff_y1[:,-1], 
    #                                 diff_x0y0[:,-1], diff_x0y1[:,-1], diff_x1y0[:,-1], diff_x1y1[:,-1]]), axis=0)
    # # diff_norm_avg = tf.reduce_mean(tf.norm(tf.stack([diff_x0, diff_y0, diff_x1, diff_y1, diff_x0y0, diff_x0y1, diff_x1y0, diff_x1y1]), axis=-1), axis=0)
    # mask_x0_diff = tf.abs(diff_x0[:,-1]) <= (2.0 * diff_depth_avg)
    # mask_y0_diff = tf.abs(diff_y0[:,-1]) <= (2.0 * diff_depth_avg)
    # mask_x1_diff = tf.abs(diff_x1[:,-1]) <= (2.0 * diff_depth_avg)
    # mask_y1_diff = tf.abs(diff_y1[:,-1]) <= (2.0 * diff_depth_avg)
    # mask_x0y0_diff = tf.abs(diff_x0y0[:,-1]) <= (2.0 * diff_depth_avg)
    # mask_x0y1_diff = tf.abs(diff_x0y1[:,-1]) <= (2.0 * diff_depth_avg)
    # mask_x1y0_diff = tf.abs(diff_x1y0[:,-1]) <= (2.0 * diff_depth_avg)
    # mask_x1y1_diff = tf.abs(diff_x1y1[:,-1]) <= (2.0 * diff_depth_avg)
    # normals0_mask = tf.cast(mask_x1_diff & mask_y1_diff, tf.float32)
    # normals1_mask = tf.cast(mask_x0_diff & mask_y0_diff, tf.float32)
    # normals2_mask = tf.cast(mask_x0y1_diff & mask_x0y0_diff, tf.float32)
    # normals3_mask = tf.cast(mask_x1y0_diff & mask_x1y1_diff, tf.float32)
